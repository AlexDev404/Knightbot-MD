...
async function handleMessages(sock, messageUpdate, printLog) {
  try {
    ...
    const shouldStop = await messageProcessor.processMessage(
      sock,
      message,
      messageData,
      context
    ...
    if (messageData.isCommand) {
      console.log(
        `📝 Command used in ${context.isGroup ? "group" : "private"}: ${
          messageData.text
        }`
      );
    }
    ...
    if (!messageData.isCommand) {
      await messageProcessor.processNonCommandMessage(
        sock,
        message,
        messageData,
        context
    ...
    try {
      ...
      console.error(
        `Command execution failed for ${messageData.command}:`,
        error
      );
    ...
    if (!commandExecuted) {
      commandExecuted = await handleLegacyCommands(
        sock,
        message,
        messageData,
        context
    ...
    await messageProcessor.postCommandProcessing(
      sock,
      message,
      context,
      commandExecuted
    ...
    console.log("[ERR_MAIN]: Error in the main thread!\n", error);
...
async function handleGroupParticipantUpdate(sock, update) {
  try {
    ...
    if (action === "add") {
      ...
      const channelId =
        welcomeData?.channelId || "120363161513685998@newsletter";
    ...
    if (action === "remove") {
      ...
      const channelId =
        goodbyeData?.channelId || "120363161513685998@newsletter";
