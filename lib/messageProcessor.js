    ...
    async processNonCommandMessage(sock, message, messageData, context) {
        try {
            ...
            if (context.isGroup && messageData.text) {
                ...
                await handleChatbotResponse(sock, context.chatId, message, messageData.text, context.senderId);
                await Antilink(message, sock);
                await handleBadwordDetection(sock, context.chatId, message, messageData.text, context.senderId);
            }
        } catch (error) {
