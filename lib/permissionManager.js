...
class PermissionManager {
  ...
  async checkPermissions(command, sock, chatId, senderId, message) {
    ...
    const participants = global.groupMetadata.participants;
    const groupAdmins = participants.filter(p => p.admin);

    // Is the sender admin?
    const isSenderAdmin = groupAdmins.some(admin => admin.id === senderId);

    if (!message.key.fromMe && !senderIsSudo && senderJid !== ownerJid && !isSenderAdmin) {
