...
async function warnCommand(sock, chatId, ctx) {
  const mentionedJids =
  ...
  try {
    ...
    // Check for replied message
      ...
      userToWarn = ctx.message.extendedTextMessage.contextInfo.participant;
    }
    ...
    // Check if we're warning ourself
    ...
    // Check if we're being fooled into warning the bot
    if (userToWarn === botLId || userToWarn === botId) {
      await sock.sendMessage(chatId, {
        text: "❌ Error: Nice try, but you cannot warn the bot!",
      });
      return;
    }
