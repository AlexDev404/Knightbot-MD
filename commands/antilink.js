...
async function handleAntilinkCommand(sock, chatId, userMessage) {
  const senderId = userMessage.key.participant || userMessage.key.remoteJid;
...
async function handleLinkDetection(
  ...
  if (shouldDelete) {
    ...
    console.log(
      `Attempting to delete message with id: ${quotedMessageId} from participant: ${quotedParticipant}`
    );
    ...
    console.log(
      "No link detected or protection not enabled for this type of link."
    );
  }
}
